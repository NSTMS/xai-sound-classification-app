...
augment_audio_files <- function(audio_files) {
  ...
  for (audio_sample in audio_files) {
    mel_spec_with_time_mask <- augment_spectrogram_with_time_mask(audio_sample$mel_spec, AUDIO_CONFIG$time_mask_param)
    extended_data <-append(extended_data, list(
        mel_spec = mel_spec_with_time_mask,
        classID = audio_sample$classID
    ))

    mel_spec_with_freq_mask <- augment_spectrogram_with_frequency_mask(audio_sample$mel_spec, AUDIO_CONFIG$freq_mask_length)
